...
def build_site(database_path: str = "data/research.db", public_dir: str = "public") -> None:
    ...
    hero = f"<header class='hero'><div><h1>Latest Daily Research Scan</h1><p>{latest_date or 'No scan has completed yet.'} - neutral screening results for real asset companies.</p></div></header>"
    ...
    (out / "comparison.html").write_text(_layout("Score Comparison", f"<header class='hero'><h1>Score Comparison</h1></header><section class='panel'><pre>{html.escape(json.dumps(history, indent=2))}</pre></section>"), encoding="utf-8")
    for score in latest:
